...
contract RecoveryWithDelayPlugin is BasePluginWithEventMetadata {
	...
	function executeFromPlugin(
		...
		if (
			...
			block.timestamp >
			uint256(announcement.executionTime) +
				uint256(announcement.validityDuration)
		) {
		...
		SafeProtocolAction memory safeProtocolAction = SafeProtocolAction(
			payable(address(safe)),
			0,
			txData
		);
		SafeRootAccess memory safeTx = SafeRootAccess(
			safeProtocolAction,
			0,
			""
		);
	 ...
	 * @param nonce A uint256 used to uniquely identify a recovery transaction.
	 */
	function cancelAnnouncementFromAccount(
		address manager,
		address prevOwner,
		address oldOwner,
		address newOwner,
	...
	function getTransactionHashData(
		...
		bytes32 transactionHash = keccak256(
			abi.encode(
				DELAYED_RECOVERY_TRANSACTION_TYPEHASH,
				recoverer,
				manager,
				account,
				prevOwner,
